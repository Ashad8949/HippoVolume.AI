...
from utils.volume_stats import Dice3d, Jaccard3d, Sensitivity3d, Specificity3d, DicePerClass
...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        self.model = UNet(num_classes=getattr(config, 'num_classes', 3))
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            # STAND-OUT: Sensitivity and specificity
            # Sensitivity (recall) - how much of the real hippocampus did we capture?
            #   Low sensitivity = under-segmentation (missing hippocampal tissue)
            # Specificity - how well did we avoid labeling background as hippocampus?
            #   Low specificity = over-segmentation (false positives)
            sens = Sensitivity3d(pred_label, x["seg"])
            spec = Specificity3d(pred_label, x["seg"])
            sensitivity_list.append(sens)
            specificity_list.append(spec)

            # STAND-OUT: Dice per class (anterior=1, posterior=2)
            dc_ant = DicePerClass(pred_label, x["seg"], 1)
            dc_post = DicePerClass(pred_label, x["seg"], 2)
            dc_anterior_list.append(dc_ant)
            dc_posterior_list.append(dc_post)
            ...
            out_dict["volume_stats"].append({
                ...
                "sensitivity": sens,
                "specificity": spec,
                "dice_anterior": dc_ant,
                "dice_posterior": dc_post
                })
            print(f"{x['filename']} Dice {dc:.4f}. {100*(i+1)/len(self.test_data):.2f}% complete")

        # STAND-OUT: Identify best and worst performing volumes
        sorted_by_dice = sorted(out_dict["volume_stats"], key=lambda v: v["dice"])
        worst_5 = sorted_by_dice[:5]
        best_5 = sorted_by_dice[-5:]
        ...
        out_dict["overall"] = {
            ...
            "mean_dice_anterior": np.mean(dc_anterior_list),
            "mean_dice_posterior": np.mean(dc_posterior_list),
            "std_dice": np.std(dc_list),
            "best_dice": np.max(dc_list),
            "worst_dice": np.min(dc_list)
        }

        out_dict["best_performing"] = best_5
        out_dict["worst_performing"] = worst_5
